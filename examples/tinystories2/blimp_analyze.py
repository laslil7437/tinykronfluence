...
from pipeline import construct_gpt2, get_tinystories_dataset, get_blimp_dataset
...
def parse_args():
    ...
    parser.add_argument(
        "--blimp_task",
        type = str,
        default = "regular_plural_subject_verb_agreement_1",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Whether to use BLiMP for task.",
    )
    parser.add_argument(
        "--use_compile",
        action="store_true",
...
class BLiMPTask(Task):
    ...
    def compute_measurement(
        ...
        return (good_log_probs - bad_log_probs).mean() 
...
def main():
    ...
    analyzer = Analyzer(
        analysis_name="tinystories_blimp",
        model=model,
        task=task,
    ...
    plt.colorbar()
    plt.savefig(f"pairwise_scores_plot_{args.blimp_task}.png")
    ...
    os.makedirs(f"results/{args.blimp_task}", exist_ok=True)
    # save the scores and indices
    torch.save(top_scores, f"results/{args.blimp_task}/top_scores.pt")
    torch.save(top_indices, f"results/{args.blimp_task}/top_indices.pt")
    torch.save(bottom_scores, f"results/{args.blimp_task}/bottom_scores.pt")
    torch.save(bottom_indices, f"results/{args.blimp_task}/bottom_indices.pt")
    # sort all scores/indices
    sorted_indices = torch.argsort(scores, dim=1, descending=True)
    sorted_scores = torch.sort(scores, dim=1, descending=True).values
    torch.save(sorted_indices, f"results/{args.blimp_task}/sorted_indices.pt")
    torch.save(sorted_scores, f"results/{args.blimp_task}/sorted_scores.pt")
